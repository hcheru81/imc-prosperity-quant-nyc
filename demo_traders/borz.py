...
PRICE_AGGRESSION = (
    0  # determines how aggressively we hunt for values above and below the spread
)

THRESHOLDS = {"over": 0, "mid": 10}
...
class Trader:
    ...
    def update_starfruit_price_history(
        ...
        lowest_sell_price = sorted(
            tradingState.order_depths["STARFRUIT"].sell_orders.keys()
        )[0]
        highest_buy_price = sorted(
            tradingState.order_depths["STARFRUIT"].buy_orders.keys(), reverse=True
        )[0]
    ...
    def get_orders(
        ...
        if product_position_limit - buying_pos > 0:  # if we have capacity
            if (
                ...
                # we had remaining capacity, we were short on product. we aim to become neutral by buying back the number of shares we were short at min(accepatable, lowest_buy_price)
    ...
    def run(self, state: TradingState):
        ...
        for product in state.order_depths:
            ...
            if product_acceptable_price is 0:
                continue
            else:
