...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        current_positions = {k: abs(v) for k, v in state.position.items()}

        # print(f"Current positions: {current_positions}")

        for product, order_depth in state.order_depths.items():
            trend = self.calculate_price_trend(product, lookback_periods=20)
