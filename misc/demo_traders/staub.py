...
class Trader:
    ...
    def calculate_orders(self, product, order_depth, our_bid, our_ask, orchild=False):
        ...
        sell_vol, best_sell_price = self.get_volume_and_best_price(
            sell_orders, buy_order=False
        )
        buy_vol, best_buy_price = self.get_volume_and_best_price(
            buy_orders, buy_order=True
        )

        logger.print(
            f"Product: {product} - best sell: {best_sell_price}, best buy: {best_buy_price}"
        )
        ...
        for ask, vol in sell_orders.items():
            if position < limit and (
                ask <= our_bid or (position < 0 and ask == our_bid + 1)
            ):
        ...
        for bid, vol in buy_orders.items():
            if position > -limit and (
                bid >= our_ask or (position > 0 and bid + 1 == our_ask)
            ):
        ...
        for product in state.order_depths:
            ...
            if product == "AMETHYSTS":
                orders += self.calculate_orders(
                    product, order_depth, 10000 - self.AME_RANGE, 10000 + self.AME_RANGE
                )
                ...
                _, best_sell_price = self.get_volume_and_best_price(
                    order_depth.sell_orders, buy_order=False
                )
                _, best_buy_price = self.get_volume_and_best_price(
                    order_depth.buy_orders, buy_order=True
                )
                ...
                if len(data.starfruit_cache) == self.STARFRUIT_CACHE_SIZE:
                    lower_bound = (
                        self.estimate_starfruit_price(data.starfruit_cache) - 2
                    )
                    upper_bound = (
                        self.estimate_starfruit_price(data.starfruit_cache) + 2
                    )
                ...
                orders += self.calculate_orders(
                    product, order_depth, lower_bound, upper_bound
                )

            elif product == "ORCHIDS":
                shipping_cost = state.observations.conversionObservations[
                    "ORCHIDS"
                ].transportFees
                import_tariff = state.observations.conversionObservations[
                    "ORCHIDS"
                ].importTariff
                export_tariff = state.observations.conversionObservations[
                    "ORCHIDS"
                ].exportTariff
                ducks_ask = state.observations.conversionObservations[
                    "ORCHIDS"
                ].askPrice
                ducks_bid = state.observations.conversionObservations[
                    "ORCHIDS"
                ].bidPrice
                ...
                orders += self.calculate_orders(
                    product, order_depth, lower_bound, upper_bound, orchild=True
                )
